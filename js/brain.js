/* ============================================================
   brain.js
   - Sends conversation to OUR backend (/api/chat), which proxies
     to Groq using the server-side GROQ_API_KEY (Vercel env var).
   ...
   - Supports optional image (base64) for vision questions.
   ============================================================ */
...
Keep replies concise (2-4 sentences) since they will often be read aloud by text-to-speech.`;
  ...
  async function ask(userText, { facts, history, image } = {}) {
    const messages = [
      { role: 'system', content: buildSystemPrompt(facts) },
      ...(history || []).map((m) => ({ role: m.role === 'assistant' ? 'assistant' : 'user', content: m.content })),
      { role: 'user', content: userText || (image ? 'Describe this image.' : '') },
    ];
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 500 && errBody.error?.includes('GROQ_API_KEY')) {
  ...
  // Very simple heuristic to catch "remember that..." / "my ... is ..." style
  // statements so they get saved as long-term facts.
